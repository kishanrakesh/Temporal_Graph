...
def compute_latest_departure(driver, source, destination):
    ...
    while not Q.is_empty():
        ...
        for record in results:
            ...
            if (vIn[0], vIn[1]) in Gt:
                ...
                if get_first(vIn)[1] - get_first(other_vIn)[1]  > 0:
                    continue
            ...
            if dest == destination:
                if not S:
                    ...
                    comp = get_first(vIn)[1] - get_first(s)[1]
                    if comp > 0:
                        S.clear()
                        S.add(vIn)
                    elif comp == 0:
...
if __name__ == "__main__":
    with GraphDatabase.driver(URI, auth=AUTH) as driver:
        ...
        results = compute_latest_departure(driver, source, destination)
